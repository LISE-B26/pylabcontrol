...
from .ni7845r_support import fpga_binder
...
class NI7845R(object):
    ...
    def device_temperature(self):
        fpga_binder.read_DeviceTemperature(self.session, self.status)
...
class FIFOAnalogInput(object):
    ...
    def start_fifo(self):
        fpga_binder.start_FIFO_AI(self._fpga.session, self._fpga.is_connected)

        # start point of acquisition
        self.time_reference = numpy.datetime64(int(time.time() * 1e6), 'us')

    def stop_fifo(self):
        fpga_binder.stop_FIFO_AI(self._fpga.session, self._fpga.is_connected)
    ...
    def read_fifo_block(self):
        ai0, ai1, ai2, times = fpga_binder.read_FIFO_conv(
            self.block_size, self._fpga.session, self._fpga.is_connected)
...
class AnalogInput(object):
    ...
    def read(self):
        return fpga_binder.int_to_voltage(
            ...
            (self._fpga.session, self._fpga.is_connected))
...
class DigitalOutput(object):
    ...
    def write(self, state):
        return getattr(fpga_binder, 'set_DIO%0d' % self._channel_number) \
            (state, self._fpga.session, self._fpga.is_connected)