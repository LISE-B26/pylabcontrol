...
def find_scripts_in_python_files(folder_name, verbose = False):
    ...
    # if there are subdirs in the folder recursively check all the subfolders for scripts
    for subdir in subdirs:
        classes_dict.update(find_scripts_in_python_files(subdir))
...
def python_file_to_b26(list_of_python_files, target_folder, verbose = False, raise_errors = False):
    if verbose:
        print('attempt to load {:d} scripts: '.format(len(list_of_python_files)))

    loaded_scripts, failed, loaded_instruments = Script.load_and_append(list_of_python_files, raise_errors=raise_errors)
