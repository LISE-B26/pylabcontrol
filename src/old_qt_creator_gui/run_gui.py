...
# todo: asign data containers to thread objects and read those datacontainers in old_gui where it is written to the guis data container
...
# todo: write generic thread function that takes arbitrary old_functions for threading
    ...
    def update_parameters(self):
        ...
        def change_parameter(parameter, value_old, value_new):
            ...
            if parameter == 'block_size':
                ...
                # these parameters are no settings actually just change some visualizations in the old_gui
                pass
...
class AcquisitionThread(QtCore.QThread):
    ...
    def __init__(self, ControlMainWindow):
        ...
        :param PI: old_lib.FPGA_PID_Loop_Simple.NI_FPGA_PI object that handles the feedback loop
        :return:
        """
    ...
    def run(self):
        ...
        while self._recording:
            ...
            try:
                ...
                print("Unexpected error:", sys.exc_info()[0])
                raise
            # just emit any signal to trigger an action in the old_gui
